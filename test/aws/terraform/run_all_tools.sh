...
find . -name "main.tf" -exec dirname {} \; | grep -v ".terraform" | while read -r test_case; do echo $test_case ; ORG_PATH=$PWD ; cd $test_case ; if [ ! -f cloudrail_results.txt ]; then terraform init ; terraform plan -out=plan.out ; cloudrail run --tf-plan plan.out --directory . | grep -v "●" | tee cloudrail_results.txt ; fi ; cd $ORG_PATH; done
